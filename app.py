...
def show_login():
    ...
    .aurora-bg {
        ...
        inset: 0;
        z-index: -10;
        background: linear-gradient(135deg, #eef2f7, #f7f9fb);
        overflow: hidden;
    }
    ...
    .aurora-bg::after {
        ...
        top:-20%;
        left:-20%;
        background: radial-gradient( circle at 30% 40%, rgba(138,180,248,.38), transparent 60% ),
                    radial-gradient( circle at 70% 60%, rgba(120,200,255,.28), transparent 65% ),
                    radial-gradient( circle at 50% 80%, rgba(100,150,255,.22), transparent 60% );
    ...
    @keyframes auroraMove {
        ...
        100% { transform: translateX(-12%) translateY(-6%) scale(1); }
    }

    /* ----------------------------------------------------------
       FLOATING PARTICLES (very minimal)
    ---------------------------------------------------------- */
    .particle {
        position: fixed;
        width: 6px;
    ...
    <!-- Particles -->
    <div class="particle" style="left:10%; animation-duration:17s; animation-delay:0s;"></div>
    <div class="particle" style="left:25%; animation-duration:20s; animation-delay:2s;"></div>
    <div class="particle" style="left:45%; animation-duration:22s; animation-delay:1s;"></div>
    <div class="particle" style="left:60%; animation-duration:19s; animation-delay:3s;"></div>
    <div class="particle" style="left:80%; animation-duration:21s; animation-delay:1.5s;"></div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown('<div class="login-title">Mindbot</div>', unsafe_allow_html=True)
        st.markdown('<div class="login-subtitle">Your Productivity Assistant</div>', unsafe_allow_html=True)
        ...
        st.markdown("""
        <div style="text-align:center; margin-top:14px; color:rgba(32,33,36,.55); font-size:12px;">
...
if tool == "-- Select a tool --":
    ...
    if uploaded_file:
        ...
        with col1:
            ...
            template_type = detect_ibm_template(io.BytesIO(file_bytes))
        ...
        with st.spinner("📄 Extracting data from PDF..."):
            try:
                ...
                if template_type == 'template2':
                    ...
                    data, header_info = extract_ibm_template2_from_pdf(io.BytesIO(file_bytes))
        ...
        if data and len(data) > 0:
            ...
            with st.expander("📋 Additional Header Information"):
                col1, col2, col3 = st.columns(3)
            ...
            with st.expander("🔍 Advanced Debug Information", expanded=False):
                ...
                if corrected_data:
                    for i, row in enumerate(corrected_data[:5]):
                        sku = row[0]
                        qty = row[2]
                        cost = row[6] if len(row) > 6 else 0
