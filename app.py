...
if tool == "-- Select a tool --":
    ...
    st.write(
        "Upload one or more Dell invoice PDFs. We'll generate a single Excel with sheet 'PRE ALERT UPLOAD'."
    )

    uploaded_files = st.file_uploader(
        "Choose Dell invoice PDF(s)",
        type=["pdf"],
    ...
    master_file = st.file_uploader(
        "Master Excel (starts header at row 9)",
        type=["xlsx"],
        key="dell_master",
    )
    ...
    if uploaded_files:
        logging.info(
            f"Uploaded {len(uploaded_files)} invoice PDF(s): "
            f"{[getattr(f, 'name', 'file') for f in uploaded_files]}"
        )

    if master_file:
        logging.info(f"Uploaded master file: {getattr(master_file, 'name', 'file')}")
        ...
        if master_file is not None:
            try:
                (
                    ...
                    orion_index,
                    po_price_index,
                ) = read_master_mapping(master_file)

                logging.info("Master file loaded and mapping created successfully.")
            except Exception as e:
                st.warning(f"Could not read master file: {e}")
                logging.error(f"Failed to read master file: {e}")

        diag: list[dict] = []

        for f in uploaded_files:
            try:
                logging.info(f"Processing invoice file: {getattr(f, 'name', 'file')}")
                ...
                logging.info(
                    f"Extracted {len(rows)} rows from {getattr(f, 'name', 'file')}"
                )

            except Exception as e:
                st.warning(f"Failed to parse {getattr(f, 'name', 'file')}: {e}")
                logging.error(f"Failed to parse {getattr(f, 'name', 'file')}: {e}")

        # ✅ FIXED: correct indentation here
        if all_rows:
            logging.info(
                f"Total extracted rows: {len(all_rows)}. Generating Excel output."
            )
            ...
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
                df.to_excel(
                    writer, sheet_name="PRE ALERT UPLOAD", index=False
                )
                ...
                # (rest of your Excel formatting + REVIEW + COMPONENT + MASTER sheets)
                # ⬅️ NO LOGIC CHANGED, ONLY INDENTATION
