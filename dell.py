...
def generate_dell_quote(
    ...
    # Cell H3 → Factor (Consolidation Fee * Total Price)
    # We'll compute Total Price after filling table rows, so for now just reserve H3
    ws["H3"].value = ""  # placeholder, will update after table is filled
    ...
    for (desc_text, qty_val, unit_val, subtotal_val) in items:
        ...
        # ---- Adj/Unit (F) = Factor / Qty
        ws[f"F{row_ptr}"].value = f"=IF(C{row_ptr}>0,$H$3/C{row_ptr},0)"
